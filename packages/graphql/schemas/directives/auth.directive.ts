...
export class isSysAdmin extends SchemaDirectiveVisitor {
  public visitFieldDefinition(field) {
    const { resolve = defaultFieldResolver } = field;
    field.resolve = async function(...args) {
      let userInfo;
      ...
      [err, authUser] = await to(
        User.findOne({ where: { id: userInfo.id, role_id: 1 } })
      );
      if (!authUser) {
        throw new Error('User not system administrator');
      }
